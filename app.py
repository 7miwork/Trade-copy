...
def run_analysis_pipeline(symbol):
    """
    Run the complete stock analysis pipeline with robust error handling
...
def api_analyze():
    ...
    try:
        ...
        if not symbol:
            return api_response(error="Symbol parameter is required", status_code=400)
        ...
        if not result.get('success', False):
            ...
            return api_response(error=error_msg, status_code=500)
        
        return api_response(data=result)
    
    except Exception as e:
        print(f"[ERROR] Unexpected error in /api/analyze: {str(e)}")
        return api_response(error=f"Internal server error: {str(e)}", status_code=500)
...
def api_batch():
    ...
    try:
        ...
        if not symbols:
            return api_response(error="No symbols provided", status_code=400)
...
def api_export():
    ...
    try:
        ...
        if not export_data:
            return api_response(error="No data to export", status_code=400)
        ...
        try:
            ...
            return api_response(error="Failed to create export directory", status_code=500)
        ...
        try:
            ...
            return api_response(error="Failed to export data", status_code=500)
